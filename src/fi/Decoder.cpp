...
void Decoder::getComment(FI::Comment& comment)
{
...
void Decoder::getAttribute(FI::Attribute &attribute)
{
  ...
  _vocab->addAttributeName(attribute._qualifiedName);
...
unsigned int Decoder::getQualifiedNameOrIndex2(FI::QualifiedNameOrIndex& name)
{
		...
		if(isPrefixPresent)
    {
      _b = static_cast<unsigned char>(_stream->get()); // next byte
			getIdentifyingStringOrIndex(name._literalQualifiedName._prefix);
    }
    ...
    {
      _b = static_cast<unsigned char>(_stream->get()); // next byte
			getIdentifyingStringOrIndex(name._literalQualifiedName._namespaceName);
    }

		// C.17.3.4 The component local-name is encoded as described in C.13.
    _b = static_cast<unsigned char>(_stream->get()); // next byte
		getIdentifyingStringOrIndex(name._literalQualifiedName._localName);
    return 0;
	}
	else 
	{
		// C.17.4 If the alternative name-surrogate-index is present, it is encoded as described in C.25.
		name._nameSurrogateIndex = decodeInteger2(); 
    return name._nameSurrogateIndex;
	}
}

// C.18: Check whether we've got a literal QNAME or surrogate
unsigned int Decoder::getQualifiedNameOrIndex3(FI::QualifiedNameOrIndex& name)
{
	...
	else
	{
		...
		name._nameSurrogateIndex = decodeInteger3(); 
    return name._nameSurrogateIndex;
	}
}

// C.12 Encoding of the NamespaceAttribute type
void Decoder::getNamespaceAttribute(FI::NamespaceAttribute &value)
{
  // An encoding of this type always starts on the eighth bit of 
  // an octet and ends on the eighth bit of another octet.
  switch(_b & Constants::LAST_TWO_BITS)
    {
    // no prefix, no namespace
    case 0:
      std::cerr << "no prefix, no namespace" << std::endl;
      break;
    // no prefix, but namespace
    case 1:
      _b = static_cast<unsigned char>(_stream->get()); // next byte
    ...
    case 2:
      _b = static_cast<unsigned char>(_stream->get()); // next byte
      getIdentifyingStringOrIndex(value._prefix);
      std::cerr << "prefix, but no namespace" << std::endl;
      break;
    // prefix and namespace
    case 3:
      _b = static_cast<unsigned char>(_stream->get()); // next byte
      if (getIdentifyingStringOrIndex(value._prefix) == INDEX_NOT_SET)
        {
        std::string prefix;
...
// C.13
unsigned int Decoder::getIdentifyingStringOrIndex(FI::IdentifyingStringOrIndex &value)
{
	...
	{
		value._stringIndex = decodeInteger2();		
    return value._stringIndex;
	}
...
void Decoder::getNonIdentifyingStringOrIndex3(FI::NonIdentifyingStringOrIndex& value)
{
	...
	else
	{
		...
		value._stringIndex = decodeInteger4();
	}
}
...
void Decoder::getEncodedCharacterString3(FI::EncodedCharacterString& value)
{
	...
	case Constants::ENCODED_CHARACTER_STRING_3RD_RESTRICTED_ALPHABET:
		...
		first = _b;
		_b = static_cast<unsigned char>(_stream->get()); // next byte
		value._index = decodeSmallInteger5(first, _b);
		break;
...
void Decoder::getEncodedCharacterString5(FI::EncodedCharacterString& value)
{
	...
	case Constants::ENCODED_CHARACTER_STRING_5TH_RESTRICTED_ALPHABET:
		...
		first = _b;
		_b = static_cast<unsigned char>(_stream->get()); // next byte
		value._index = decodeSmallInteger7(first, _b);
		break;
...
// C.25 & C.26
unsigned int Decoder::decodeInteger2()
{
	unsigned int iValue;
	char buf[2];
...
// C.27
unsigned int Decoder::decodeInteger3()
{
	unsigned int iValue;
	char buf[3];
...
// C.28
unsigned int Decoder::decodeInteger4()
{
	unsigned int iValue;
	char buf[3];
...
// C.29
unsigned int Decoder::decodeSmallInteger5(unsigned char first, unsigned char second) const
{
	return ((first & Constants::LAST_FOUR_BITS) << 4) + ((second & Constants::FOUR_BITS) >> 4) + 1;
}

// C.29
unsigned int Decoder::decodeSmallInteger7(unsigned char first, unsigned char second) const
{
	return (((first & Constants::LAST_TWO_BITS) << 6) + ((second & Constants::SIX_BITS) >> 2) + 1);
}
